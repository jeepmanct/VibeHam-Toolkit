...
statsRoutes.get('/distance', (c) => {
  ...
  if (!home) {
    c.header('Cache-Control', 'no-store');
    return c.json({ longest: null, buckets: [] });
  }
...
// Hour-of-day and day-of-week breakdowns, both in UTC (ADIF's native
...
statsRoutes.get('/activity-pattern', (c) => {
  const hourRows = db
    .query(`SELECT substr(time_on, 1, 2) as hour, COUNT(*) as count FROM qsos WHERE time_on IS NOT NULL GROUP BY hour`)
    .all() as { hour: string; count: number }[];
  const hourByNum = new Map(hourRows.map((r) => [Number(r.hour), r.count]));
  const byHour = Array.from({ length: 24 }, (_, h) => ({ hour: h, count: hourByNum.get(h) ?? 0 }));

  const dowRows = db
    .query(
      `SELECT CAST(strftime('%w', substr(qso_date,1,4) || '-' || substr(qso_date,5,2) || '-' || substr(qso_date,7,2)) AS INTEGER) as dow,
              COUNT(*) as count
       FROM qsos GROUP BY dow`,
    )
    .all() as { dow: number; count: number }[];
  const dowByNum = new Map(dowRows.map((r) => [r.dow, r.count]));
  const byDayOfWeek = DAY_NAMES.map((name, i) => ({ day: name, count: dowByNum.get(i) ?? 0 }));

  c.header('Cache-Control', 'no-store');
  return c.json({ byHour, byDayOfWeek });
});
