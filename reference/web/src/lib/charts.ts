...
export function renderLineChart(container: HTMLElement, data: LinePoint[], color: string, opts: { minY?: number; suffix?: string } = {}) {
  const width = container.clientWidth || 400;
  const height = 180;
  ...
  data.forEach((d) => {
    svg += `<circle cx="${xScale(d.x).toFixed(1)}" cy="${yScale(d.y).toFixed(1)}" r="2.5" fill="${color}"><title>${d.x}: ${d.y.toFixed(1)}${opts.suffix ?? ''}</title></circle>`;
  });
  ...
  svg += `<text x="${padding.left}" y="${height - 4}" font-size="10" fill="var(--muted)">${minX}</text>`;
  svg += `<text x="${width - padding.right}" y="${height - 4}" font-size="10" fill="var(--muted)" text-anchor="end">${maxX}</text>`;
  svg += '</svg>';
  container.innerHTML = svg;
}
